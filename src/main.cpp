      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // update and compute steering value at each step
          pid_steering.UpdateError(cte);
          steer_value = -(pid_steering.Kp * pid_steering.p_error + pid_steering.Ki * pid_steering.i_error + pid_steering.Kd * pid_steering.d_error);

          // update and compute throttle value at each step
          pid_throttle.UpdateError(fabs(cte)); // float abs
          throttle_value = 0.76 -(pid_throttle.Kp * pid_throttle.p_error + pid_throttle.Ki * pid_throttle.i_error + pid_throttle.Kd * pid_throttle.d_error);
          ...
          // case 1: limit max throttle at around 30 mph.
